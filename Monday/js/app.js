...
class bankAccount {
  ...
  deposit(amount) {
    this.balance += amount;
    console.log(
      `Account of ${this.firstName} has new balance of ${this.balance}`
    );
  }
  withdraw(amount) {
    this.balance -= amount;
  ...
  chargeFee() {
    if (this.balance < 0) {
      this.fees = "$-35";
      console.log(
        `A free of $35 has been charged to this account of ${this.firstName}`
      );
    }
  }
}
let newAccount = new bankAccount("John", "Doe", "", "checking", 50, "Opened");
let secondAccount = new bankAccount(
  "Marry",
  "Doe",
  "T",
  "Checking",
